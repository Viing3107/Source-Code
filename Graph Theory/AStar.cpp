...
int a[100][100], n, start, finish, b[100];
Node p[100], Open[100], Close[100];

void ReadInputFile1(int *b, int &n){
	freopen("Input1.txt", "r", stdin);
	if (!fopen("Input1.txt", "r")) cout << "Khong the mo duoc file!";
	else{
		cin >> n;
...
void ReadInputFile2(int a[100][100], int &n, int &start, int &finsh){
	...
	if (!fopen("Input2.txt", "r")) cout << "Khong the mo duoc file!";
	else{
		cin >> n >> start >> finsh;
		for (int i = 0; i < n; i++){
		...
		if (FindPoint(n, p, Open[k].index) == finsh){
			...
			for (int i = 0; i < n; i++){
				...
				if (a[FindPoint(n, p, Open[k].index)][i] != 0 && p[i].color == 1){  // neu dinh da co trong Open
					...
					if (tempNode.f < Open[h].f) Open[h] = tempNode;   // neu f trang thai hien tai be hon trang thai cap nhat truoc do
				}
